...
func (s *Service) registerModelsForAuthWithCache(ctx context.Context, a *coreauth.Auth, compatCache *openAICompatibilityRegistrationCache) {
	...
	case "xai":
		...
		models = applyExcludedModels(models, excluded)
	case "qodercn":
		// Live list preferred; falls back to static catalog without credentials.
		models = executor.FetchQoderCNModels(ctx, a, s.cfg)
		models = applyExcludedModels(models, excluded)
	case "qoder":
...
func buildCommandCodeConfigModels(entry *config.CommandCodeKey) []*ModelInfo {
	...
	return buildConfigModels(entry.Models, constant.CommandCode, constant.CommandCode, constant.CommandCode)
}
