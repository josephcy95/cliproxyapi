...
func TestServiceShutdownClosesIdleServer(t *testing.T) {
	...
	// A connected but silent TCP peer is not an idle HTTP keep-alive request.
	// Close the readiness probe before testing graceful service shutdown.
	_ = conn.Close()
	conn = nil
	defer func() {
		if conn != nil {
	...
	if stopDuration >= time.Second {
		t.Fatalf("service.Shutdown took %v, want prompt shutdown of an idle server", stopDuration)
	}
	if errShutdown != nil {
...
func TestServiceRun_ShutdownContextFreshAtExit(t *testing.T) {
	...
	// Reproduce the original stale-context defect, not just startup/stop smoke.
