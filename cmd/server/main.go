...
func main() {
	...
	flag.BoolVar(&qodercnLogin, "qodercn-login", false, "Login to Qoder CN using OAuth device flow")
	flag.BoolVar(&qoderLogin, "qoder-login", false, "Login to Qoder (international) using OAuth device flow")
	...
	commandMode := vertexImport != "" || antigravityLogin || codexLogin || codexDeviceLogin || claudeLogin || kimiLogin || kimiAILogin || xaiLogin || qodercnLogin || qoderLogin || devinLogin || metaLogin
...
// Codex client templates, the model catalog fallback, and Devin catalogs still
// refresh under Home mode because the model list comes from Home IDs while
// template metadata, name-based metadata, and Devin models stay edge-local.
func modelCatalogUpdaterPlan(localModel, homeEnabled bool) (startModels, startCodexClient, startCatalogFallback, startDevin bool) {
	if localModel {
...
func startModelCatalogUpdaters(localModel, homeEnabled bool) {
	startModels, startCodexClient, startCatalogFallback, startDevin := modelCatalogUpdaterPlan(localModel, homeEnabled)
	if startCodexClient {
