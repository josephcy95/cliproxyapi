...
func (h *Handler) ListAuthFiles(c *gin.Context) {
	...
	for _, auth := range auths {
		...
		if entry := h.buildAuthFileEntryWithAdaptive(auth, adaptiveSnapshot, quotaSupportedProviders); entry != nil {
			entry["cooldowns"] = nil
			if cooldownsKnown {
				entry["cooldowns"] = coreauth.CooldownSnapshotForAuth(auth, observedAt)
			}
...
func (h *Handler) buildAuthFileEntry(auth *coreauth.Auth, quotaSupported ...map[string]struct{}) gin.H {
	return h.buildAuthFileEntryWithAdaptive(auth, nil, firstQuotaSupported(quotaSupported...))
}

func firstQuotaSupported(quotaSupported ...map[string]struct{}) map[string]struct{} {
	if len(quotaSupported) == 0 {
		return nil
	}
	return quotaSupported[0]
}

func (h *Handler) buildAuthFileEntryWithAdaptive(auth *coreauth.Auth, adaptive map[string]coreauth.CodexAdaptiveCandidateInfo, quotaSupported map[string]struct{}) gin.H {
	authFileEntryMu.Lock()
	defer authFileEntryMu.Unlock()
	entry := h.buildAuthFileEntryLocked(auth, quotaSupported)
...
func (h *Handler) buildAuthFileEntryLocked(auth *coreauth.Auth, quotaSupported ...map[string]struct{}) gin.H {
	...
	// Expose Qoder (international) credit usage when available (filled by model/usage refresh).
