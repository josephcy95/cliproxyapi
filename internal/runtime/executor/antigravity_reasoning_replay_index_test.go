...
func TestAntigravityReplayIndexInitializesZeroValueTracker(t *testing.T) {
	tracker := &antigravityReplayContextHashLogTracker{}
	index := newAntigravityReplayRequestIndexWithTracker([]byte(`{"request":{"contents":[]}}`), tracker)
	if index.logTracker != tracker || tracker.loggedKeys == nil {
		t.Fatal("zero-value tracker was not initialized")
	}
	tracker.loggedKeys[antigravityReplayPartKey{}] = true
}
